...
// In-memory data store for resilience when DB is not connected or initialized
let currentRunState = {
  id: 1,
  cycleMonth: 9,
...
// Seed employees and payslip calculations matching mockup exactly
let employeePayslips = [
  {
    ...
    basicSalary: 30000,
    allowances: 12500, // HRA, Medical, Conveyance
    bonus: 2500,
    overtime: 1750,
    grossSalary: 46750,
    deductions: 3850, // Provident fund, professional tax
    nbrTax: 0,
    loanEmi: 4375,
    netSalary: 38525,
...
// Audit trail for revisions (SAL-07)
...
class SalaryService {
  /**
   * SAL-01/02/03: Get current payroll run data
   * Attempts to query MySQL (Employee, Department, PayrollRun, Payslip),
   * falls back to in-memory state if database is offline.
   */
  async getCurrentRun() {
    try {
      if (pool) {
        ...
        // Query employees and their payroll breakdown from MySQL database
  ...
  /**
   * SAL-03: Get net salary calculation for a specific employee
   */
  async getEmployeeCalculation(employeeId) {
    const id = parseInt(employeeId, 10);
    const employee =
      employeePayslips.find(e => e.id === id || e.employeeId === id) ||
      employeePayslips[0];

    // SAL-01/SAL-02: gross = Basic + Allowances + Bonus + OT
    const gross =
    ...
    // SAL-03: net = gross - deductions - loan EMI - NBR tax
    const net =
      gross -
      Number(employee.deductions) -
      Number(employee.loanEmi) -
      Number(employee.nbrTax);
  ...
  /**
   * SAL-04/SAL-05/SAL-07: Transition status (Draft -> Approved -> Locked)
   */
  async updateStatus(newStatus, user = 'Finance Manager') {
    const validStatuses = ['Draft', 'Approved', 'Locked'];
    if (!validStatuses.includes(newStatus)) {
      throw new Error(`Invalid status: ${newStatus}`);
    }

    // SAL-04/SAL-05: A locked run is NOT editable
    ...
    // Transition rules: Draft -> Approved -> Locked
    ...
    // SAL-07: salary revision history retained
    salaryRevisionHistory.push({
      ...
      note: `Payroll run status changed to ${newStatus}`
    });
  ...
  /**
   * SAL-06: Individual PDF payslip per employee
   */
  async generatePayslipPdf(employeeIdentifier = 'AviramSingha') {
    ...
    if (!employee) {
      employee = employeePayslips[1]; // Default to Aviram Singha
    }
    ...
    // Generate clean PDF binary stream with exact payroll slip details
